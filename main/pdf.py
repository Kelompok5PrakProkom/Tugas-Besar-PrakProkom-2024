...
images = {
    "Lapangan": load_image("Fasilitas/lapangan.jpg", (150, 150)),
    "Ruang Multimedia": load_image("Fasilitas/multimedia.jpg", (150, 150)),
    "Ruang Seminar": load_image("Fasilitas/seminar.jpg", (150, 150)),
    "Working Space": load_image("Fasilitas/workingspace.jpg", (150, 150))
}
...
def generate_invoice(facility, date, start_time, end_time, email):
    invoice = FPDF()
    invoice.add_page()
    invoice.set_font("Arial", size=12)
    invoice.cell(200, 10, txt="INVOICE", ln=True, align='C')
    invoice.cell(200, 10, txt=f"Facility: {facility}", ln=True)
    invoice.cell(200, 10, txt=f"Date: {date}", ln=True)
    invoice.cell(200, 10, txt=f"Start Time: {start_time}", ln=True)
    invoice.cell(200, 10, txt=f"End Time: {end_time}", ln=True)
    invoice.cell(200, 10, txt=f"Email: {email}", ln=True)
    
    invoice_file = f"invoice_{facility}_{date}.pdf"
    invoice.output(invoice_file)
    
    send_email(email, invoice_file)

# Fungsi untuk mengirim email dengan lampiran invoice
def send_email(recipient, invoice_file):
    sender_email = "your_email@example.com"
    sender_password = "your_password"
    subject = "Your Booking Invoice"
    body = "Attached is your booking invoice."
    ...
    with open(invoice_file, "rb") as attachment:
        part = MIMEApplication(attachment.read(), Name=invoice_file)
    part['Content-Disposition'] = f'attachment; filename="{invoice_file}"'
    msg.attach(part)

    try:
        server = smtplib.SMTP("smtp.example.com", 587)  # Change this to your email provider's SMTP server
        server.starttls()
